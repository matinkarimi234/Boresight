...
def main():
    ...
    # --- Camera ---
    camera = CameraSetup()  # rotation=180 per your Camera_Setup.py
    camera.camera.zoom = (0.0, 0.0, 1.0, 1.0)  # reset zoom

    # --- Reticle overlay ---
    overlay_display = OverlayDisplay(
        radius=20, tick_length=300, ring_thickness=1, tick_thickness=1, gap=-10,
        color=OVERLAY_COLOR
    )
    overlay_display.set_style(
        scale_spacing=10, scale_major_every=5,
        scale_major_length=15, scale_minor_length=5,
        scale_label_show=False, scale_tick_thickness=1
    )
    overlay_display.refresh()
    print(f"[boot] disp={overlay_display.disp_width}x{overlay_display.disp_height}", flush=True)

    # Tell camera the real display aspect
    camera.set_display_aspect(overlay_display.disp_width, overlay_display.disp_height)

    # Start preview in exact display window (fallback to fullscreen if needed)
    try:
    ...
    # If left/right feels reversed on your stack, use 'inverse'. You said this is fine.
    ...
    clock_overlay = TextOverlay(
        layer=2002, font_path="Fonts/Tw_Cen_Condensed.ttf", font_size=36,
        pos=('left', 'bottom'), color=OVERLAY_COLOR, offset=20
    )

    calender_overlay = TextOverlay(
        layer=2003, font_path="Fonts/Tw_Cen_Condensed.ttf", font_size=36,
        pos=('right', 'bottom'), color=OVERLAY_COLOR, offset=20
    )

    state_overlay = TextOverlay(
        layer=2004, font_path="Fonts/Tw_Cen_Condensed.ttf", rec_color=OVERLAY_COLOR,
        font_size=36, pos=('right', 'top'), color=OVERLAY_COLOR, offset=20
    )

    static_png = StaticPNGOverlay("Pictures/Farand_Logo.png", layer=2005,
                                  pos=('left','top'), scale=0.35, offset=20)
    static_png.show()

    record_manager = RecordingManager(base_dir="/home/boresight/Saved_Videos")

    # ---- Zoom/reticle state ----
    prezoom_reticle_px = None         # (x_px, y_px) saved when going 1× -> >1× (only used if user didn't move)
    ...
    def _ensure_anchor():
        """Make sure zoom_anchor_sensor is set to the current reticle world point."""
        global zoom_anchor_sensor
        if zoom_anchor_sensor is None:
            nx, ny = overlay_display.reticle_norm_on_display()   # NOTE: your overlay already returns (1-nx, 1-ny)
            sx, sy = camera._display_to_sensor_forward(nx, ny)   # maps display -> sensor w/ rotation=180 awareness
    ...
    def state_machine_thread():
        ...
        global arrow_buttons_press_duration, exit_buttons_press_duration
        global zoom_Step, button_ok_pressed
        global prezoom_reticle_px, current_zoom, zoom_anchor_sensor, zoom_anchor_dirty

        reticle_STEP = 1  # px per nudge
        ...
        while getattr(state_machine, "running", True):
            ...
            if current_state == StateMachineEnum.START_UP_STATE:
                ...
                if button_left_up_pressed and not button_right_down_pressed and not button_ok_pressed:
                    if current_zoom == 1:
                        # Save pre-zoom overlay pixel pos (only used if user doesn't move while zoomed)
                        prezoom_reticle_px = overlay_display.get_center()
                        # Create anchor from current reticle
                ...
                if button_right_down_pressed and not button_left_up_pressed and not button_ok_pressed:
                    ...
                    if current_zoom > 1:
                        ...
                        if zoom_anchor_sensor and zoom_anchor_dirty:
                            # place reticle at correct 1× position of the same world target
                            ...
                            # user didn't move while zoomed: restore pre-zoom pixel location
                            restore_overlay_reticle()

                        # clear state ONLY when leaving zoom
                ...
                # While zoomed, update world anchor to the moved reticle
                if current_zoom > 1:
